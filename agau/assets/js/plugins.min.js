...
class InputMasking{
   ...
   constructor(el, format){
       ...
       var FormatMatches = this.format.match(/(X+)/gi)
       var _altFormat = this.format;

       //Default Input Pattern RegExp Value
       var inputFormat = '^' + this.format.replace(/[!@#$%^&*()+=\[\]\\';,./{}|":<>?~]/g, "(\\$&)").replace(/\s/, "\\s")


       FormatMatches.forEach((pat, i) =>{
           //Set Matched Group with the pattern
           this.pattern[i+1]= `(\\d{${pat.length}})`
           //Replace Character Group to a Replacement Pattern
           _altFormat= _altFormat.replace(pat, `\$${i+1}`);
           //Replace Character Group to RegExp Pattern as Input Patter Attribue
           inputFormat = inputFormat.replace(pat, `(\\d{${pat.length}})`);
       })
       // Join Pattern Array
       ...
       //Disable input's autocomplete
       this.el.setAttribute('autocomplete', 'off')

       //Trigger Input Masking / Format Value
       this.el.addEventListener('input', this.mask.bind(this))
       //Check if the current input value is equal to the complere require pattern/format
       this.el.addEventListener('keydown',  e => {this.checkVal(e,this)})
   }
   mask(){
       //Re-format input value according to the given pattern
       this.el.value = String(this.el.value).replace(this.pattern, this._alt);
   }
   checkVal(e, _this){
       if( e.key.length == 1 && _this.el.value.length >= _this.format.length){
           //Stop adding new character if the input already meet the required pattern
           e.preventDefault()
           return false
       }
   }
